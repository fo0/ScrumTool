...
public class MarkDownDialog extends Dialog {
  ...
  private Button createBtnCommonMarkdown() {
    ...
    btnCommonMarkdown.addClickListener(e -> {
      area.setValue(VanillaMarkDown.create(data)
                                   .stream()
  ...
  private Button createBtnJson() {
    Button btnCommonMarkdown = new Button("Create Json");
    btnCommonMarkdown.addClickListener(e -> {
      area.setValue(ExportJson.exportAsJson(data.getId()));
    });
    return btnCommonMarkdown;
  }
